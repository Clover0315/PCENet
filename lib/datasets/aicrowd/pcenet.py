...
class Dataset(data.Dataset):
    ...
    def get_extreme_4points(self, instance_polys):
        '''从poly获取4极点'''
        extreme_points = []
        for instance in instance_polys:
    ...
    def get_extreme_points(self, instance_polys):
        extreme_points = []
        for instance in instance_polys:
            points = [snake_whu_utils.get_extreme_points(poly) for poly in instance]
            extreme_points.append(points)
        return extreme_points
    ...
    def prepare_init(self, box, extreme_point, i_it_4pys, c_it_4pys, i_gt_4pys, c_gt_4pys,h, w):
        ...
        img_init_poly = snake_coco_utils.uniformsample(img_init_poly, snake_config.init_poly_num)  # 均匀重采样
        img_gt_poly = snake_coco_utils.uniformsample(extreme_point, snake_config.init_poly_num)  # 均匀重采样
        ...
        can_gt_poly = can_poly(img_gt_poly, x_min, y_min, x_max, y_max)
    ...
    def prepare_evolution(self, poly, extreme_point, img_gt_polys, can_gt_polys):
        x_min, y_min = np.min(extreme_point[:, 0]), np.min(extreme_point[:, 1])
        x_max, y_max = np.max(extreme_point[:, 0]), np.max(extreme_point[:, 1])
    ...
    def __getitem__(self, index):
        ...
        c_it_4pys = []
        i_gt_4pys = []  # extreme points，作为真值 （gt：ground truth真实值）
        c_gt_4pys = []
        ...
        for i in range(len(anno)):
            ...
            for j in range(len(instance_poly)):
                ...
                # 遍历更新 4init 的 4个数组、8init的4个数组
                self.prepare_init(decode_box, extreme_point, i_it_4pys, c_it_4pys, i_gt_4pys, c_gt_4pys,
                                  output_h, output_w)
                # 遍历更新 evolution 的 4个数组
                self.prepare_evolution(poly, extreme_point, i_gt_pys, c_gt_pys)
                gt_pys.append(poly)

        ret = {'inp': inp}
        detection = {'ct_hm': ct_hm, 'wh': wh, 'reg': reg, 'ct_cls': ct_cls, 'ct_ind': ct_ind}
        # i_it_4py 40点，c_it_4py 40点，i_gt_4py 极点， c_gt_4py 极点平移
        init4 = {'i_it_4py': i_it_4pys, 'c_it_4py': c_it_4pys, 'i_gt_4py': i_gt_4pys, 'c_gt_4py': c_gt_4pys}
        # i_it_py 64点  c_it_py 64点， i_gt_py 128点，  c_gt_py 128点
        evolution = {'i_gt_py': i_gt_pys, 'c_gt_py': c_gt_pys}
        ret.update(detection)
        ret.update(init4)
        ret.update(evolution)
        # ret.update(endpoint_type)
...
def visual_datalist(img, data_list):
    ...
    for _data in data_list:
        ...
        # ax.plot(x_data * 4, y_data * 4, linewidth=2, color='green')
        # ax.scatter(x_data * 4, y_data * 4, s=8, edgecolors='w')

        ax.fill(x_data, y_data, color='red', alpha=0.3, zorder=0)
        ax.plot(x_data, y_data, linewidth=1, color='red', zorder=1)
        ax.scatter(x_data, y_data, s=8,  c='red', marker='o',  zorder=2, edgecolor='w')
...
if __name__ == "__main__":
    # dataset_path = "/home/ai-center/clover/project/building-extraction/snake-master/data/aicrowd/train/"
    # # dataset_path = "/home/ai-center/clover/project/building-extraction/snake-master/data/aicrowd/val/val"
    # ann_file = os.path.join(dataset_path, "annotation.json")
    # data_root = os.path.join(dataset_path, "images")  # 图片地址
    ...
    jpg_name ='800000106575.tif'
    jpg_path = os.path.join(data_root, jpg_name)
    annId = dataset.coco.getAnnIds([800000106575])
    anns = dataset.coco.loadAnns(annId)

    img = cv2.imread(jpg_path)  # 获取图片ndarray h,w,c
    img = cv2.cvtColor(img, cv2.COLOR_BGR2RGB)

    datalist = []
    for _ in anns:
        points = _['segmentation'][0]
        x_point = points[::2]
        y_point = points[1::2]
        poly = np.array([x_point, y_point]).transpose(1, 0)

        # box
        # x_min, y_min = np.min(poly[:, 0]), np.min(poly[:, 1])
        # x_max, y_max = np.max(poly[:, 0]), np.max(poly[:, 1])
        # bbox = [x_min, y_min, x_max, y_max]
        # arr = snake_whu_utils.get_init(bbox)

        # 外接多边形
        arr = snake_whu_utils.get_extreme_points(poly)

        #
        # arr = snake_coco_utils.uniformsample(poly, 60)
        datalist.append(arr)
    visual_datalist(img, datalist)
