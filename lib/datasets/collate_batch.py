...
def snake_collator(batch):
    ...
    i_gt_4pys = torch.zeros([batch_size, ct_num, snake_config.ex_gt_num, 2], dtype=torch.float)
    c_gt_4pys = torch.zeros([batch_size, ct_num, snake_config.ex_gt_num, 2], dtype=torch.float)
    if ct_num != 0:
...
def snake_ep_tp_collator(batch):
    ret = {'inp': default_collate([b['inp'] for b in batch])}
    ...
    # endpoint class
    gt_pys = torch.zeros([batch_size, ct_num, snake_config.poly_num, 2], dtype=torch.float)
    gt_pys_num = torch.zeros([batch_size, ct_num], dtype=torch.float)
    if ct_num != 0:
        gt_pys[ct_01] = torch.Tensor(sum([b['gt_pys'] for b in batch], []))
        gt_pys_num[ct_01] = torch.Tensor(sum([b['gt_pys_num'] for b in batch], []))
    endpoint_type = {'gt_pys': gt_pys, 'gt_pys_num': gt_pys_num}
    ret.update(endpoint_type)
    return ret


def snake3_collator(batch):
    ret = {'inp': default_collate([b['inp'] for b in batch])}
    ...
    i_it_4pys = torch.zeros([batch_size, ct_num, snake_config.init_poly_num, 2], dtype=torch.float)
    c_it_4pys = torch.zeros([batch_size, ct_num, snake_config.init_poly_num, 2], dtype=torch.float)
    i_gt_4pys = torch.zeros([batch_size, ct_num, snake_config.init_poly_num, 2], dtype=torch.float)
    # c_gt_4pys = torch.zeros([batch_size, ct_num, snake_config.init_poly_num, 2], dtype=torch.float)
    if ct_num != 0:
        i_it_4pys[ct_01] = torch.Tensor(sum([b['i_it_4py'] for b in batch], []))
        c_it_4pys[ct_01] = torch.Tensor(sum([b['c_it_4py'] for b in batch], []))
        i_gt_4pys[ct_01] = torch.Tensor(sum([b['i_gt_4py'] for b in batch], []))
        # c_gt_4pys[ct_01] = torch.Tensor(sum([b['c_gt_4py'] for b in batch], []))
    init4 = {'i_it_4py': i_it_4pys, 'c_it_4py': c_it_4pys,
             'i_gt_4py': i_gt_4pys,
             # 'c_gt_4py': c_gt_4pys
             }
    ret.update(init4)
...
def snake3_ep_tp_collator(batch):
    ret = {'inp': default_collate([b['inp'] for b in batch])}
...
def make_collator(cfg):
    if cfg.task in _collators:
        ...
        if cfg.task == 'snake_ep_tp':
            if cfg.snake == 'snake':
                return snake_ep_tp_collator
            else:
                return snake3_ep_tp_collator
