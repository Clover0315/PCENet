...
class Evolution(nn.Module):
    ...
    def init_poly(self, snake, cnn_feature, i_it_poly, c_it_poly, ind):
        ...
        i_poly = i_poly[:, ::snake_config.init_poly_num//4]  # 获得4个顶点
    ...
    def forward(self, output, cnn_feature, batch=None):
        ...
        if batch is not None and 'test' not in batch['meta']:
            ...
            py_pred = self.evolve_poly(self.evolve_gcn, cnn_feature, init['i_it_py'], init['c_it_py'], init['py_ind'], 0)
            py_preds = [py_pred]
        ...
        if not self.training:
            with torch.no_grad():
                ...
                # 通过snake 获取初始菱形的offset，ex[极点] = 初始四边形+offset
                ...
                py = self.evolve_poly(self.evolve_gcn, cnn_feature, evolve['i_it_py'], evolve['c_it_py'], init['ind'], 0)
                pys = [py / snake_config.ro]
                for i in range(self.iter):
                    py = py / snake_config.ro
                    c_py = snake_gcn_utils.img_poly_to_can_poly(py)  # 多边形平移到边缘
                    evolve_gcn = self.__getattr__('evolve_gcn'+str(i))
