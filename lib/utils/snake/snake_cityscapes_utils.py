...
def get_extreme_points(pts):
    ...
    t_idxs = [t_idx]
    '''与t连续相邻，且y值与t差值小于阈值的点'''
    # 从t开始向前遍历pts，端点的y值与t的差值小于阈值的append到t_idxs数组中，不符合条件则跳出循环
    tmp = (t_idx + 1) % vertex_num  # 计算前一个端点的索引号 (t索引+1）% 点数 值域在0~vertex_num-1 之间，避免索引溢出，且使得收尾相连[边界多边形是一个环]
    while tmp != t_idx and pts[tmp, 1] - t <= thresh * h:  # tmp不是t_idx，且二者差值在阈值内时
        t_idxs.append(tmp)
        tmp = (tmp + 1) % vertex_num
    # 从t开始向后遍历pts，端点的y值与t的差值小于阈值的append到t_idxs数组中，不符合条件则跳出循环
    tmp = (t_idx - 1) % vertex_num
    while tmp != t_idx and pts[tmp, 1] - t <= thresh * h:
        t_idxs.append(tmp)
        tmp = (tmp - 1) % vertex_num
    #
