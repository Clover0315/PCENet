...
def prepare_testing_init(box, score):
    ...
    i_it_4pys = uniform_upsample(i_it_4pys, snake_config.init_poly_num)    # 每条边上均匀采样
    c_it_4pys = img_poly_to_can_poly(i_it_4pys)  # 平移平移到边缘（左下角）
...
def prepare_training(ret, batch):
    ...
    if 'i_it_min_4py' in batch:
        init.update({'i_it_min_4py': collect_training(batch['i_it_min_4py'], ct_01)})
        init.update({'c_it_min_4py': collect_training(batch['c_it_min_4py'], ct_01)})
        init.update({'i_gt_min_4py': collect_training(batch['i_gt_min_4py'], ct_01)})
        # init.update({'c_gt_min_4py': collect_training(batch['c_gt_min_4py'], ct_01)})
...
def prepare_testing_evolve(ex):
    if len(ex) == 0:
        ...
        i_it_pys = snake_decode.get_octagon(ex[None])  # 八边形
        i_it_pys = uniform_upsample(i_it_pys, snake_config.poly_num)[0]  # 均匀采样
        c_it_pys = img_poly_to_can_poly(i_it_pys)  # 八边形平移到边缘
    evolve = {'i_it_py': i_it_pys, 'c_it_py': c_it_pys}
    return evolve


def get_gcn_feature(cnn_feature, img_poly, ind, h, w):
    # img_poly 多边形数目*节点数*2  # 把一个批的所有多边形放在一个矩阵里
    # cnn_feature b*c*w*h 1*64*80*80
    # ind img_poly 属于哪一个批
    img_poly = img_poly.clone()
    # 归一化，将图像的坐标范围从[-h/2, h/2]映射到[-1, 1]
    ...
    for i in range(batch_size):
        ...
        # 从feature上，取poly 所在像素点的特征值 第i批的多边形数目*chanel*节点数
        # grid的元素值通常在[-1,1]之间，(-1,-1) 表示取输入特征图左上角的元素，(1,1) 表示取输入特征图右下角的元素。
        feature = torch.nn.functional.grid_sample(cnn_feature[i:i+1], poly)[0].permute(1, 0, 2)  # 网格采样
